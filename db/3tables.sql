...
-- -----------------------------------------------------
-- Table `pajax1`.`lesavatars`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pajax1`.`lesavatars` ;

CREATE TABLE IF NOT EXISTS `pajax1`.`lesavatars` (
  `idlesavatars` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `lurl` VARCHAR(100) NULL,
  `lesutilisateurs_idutilisateur` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`idlesavatars`),
  INDEX `fk_lesavatars_lesutilisateurs1_idx` (`lesutilisateurs_idutilisateur` ASC),
  CONSTRAINT `fk_lesavatars_lesutilisateurs1`
    FOREIGN KEY (`lesutilisateurs_idutilisateur`)
    REFERENCES `pajax1`.`lesutilisateurs` (`idutilisateur`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
